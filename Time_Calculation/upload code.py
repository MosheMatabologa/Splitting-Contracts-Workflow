...
df['Clock Out Time'] = df['Clock Out Time'].astype(str).str.replace('24:00', '00:00:00')
df['Clock In Time'] = df['Clock In Time'].astype(str).str.replace('24:00', '00:00:00')

df['Clock Out Time'] = pd.to_timedelta(df['Clock Out Time'], errors='coerce')
df['Clock In Time'] = pd.to_timedelta(df['Clock In Time'], errors='coerce')
...
df.to_excel("123IneedM7MsRaymondeMosheSheet1Output.xlsx", index=False, engine='xlsxwriter')